...
public class IDGeneratorUtil {
    ...
    private IDGeneratorUtil(long workerId) {
    ...
    private static IDGeneratorUtil flowIdWorker = new IDGeneratorUtil(1);

    public static IDGeneratorUtil getFlowIdWorkerInstance() {
        return flowIdWorker;
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        IDGeneratorUtil idWorker = IDGeneratorUtil.getFlowIdWorkerInstance();
        System.out.println(idWorker.nextId());
        for (int i = 0; i < 100; i++) {
            System.out.println((i + 1) + " = " + (idWorker.nextId() + "").substring(2));
        }
    }
